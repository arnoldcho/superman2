# PSBT Howto for Superman Core

Since Superman Core 0.17, an RPC interface exists for Partially Signed Superman
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/superman/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for Superman transactions that are not fully signed
...
#### Multisig with multiple Superman Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Superman Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `superman-cli` is used.
...
Later, when *V* SPM has been received on *Amulti*, and Bob and Carol want to
