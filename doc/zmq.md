...
The Superman Core daemon can be configured to act as a trusted "border
router", implementing the superman wire protocol and relay, making
...
The ZeroMQ feature in Superman Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building supermand:
...
For instance:
    ...
    $ supermand -zmqpubhashtx=tcp://127.0.0.1:29332 \
               -zmqpubhashtx=tcp://192.168.1.2:29332 \
               -zmqpubrawtx=ipc:///tmp/supermand.tx.raw \
...
These options can also be provided in superman.conf.
...
From the perspective of supermand, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into supermand directly. Furthermore, no information is
...
using. Supermand appends an up-counting sequence number to each
