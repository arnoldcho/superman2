...
static void SetupSupermanTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " superman-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  superman-tx [options] <hex-tx> [commands]  Update hex-encoded superman transaction\n" +
            "or:     superman-tx [options] -create [commands]   Create hex-encoded superman transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded superman transaction
