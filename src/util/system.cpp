...
const char * const SUPERMAN_CONF_FILENAME = "superman.conf";
const char * const SUPERMAN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/superman/superman/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //superman-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", SUPERMAN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Superman
    // macOS: ~/Library/Application Support/Superman
    // Unix-like: ~/.superman
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Superman";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Superman";
#else
    // Unix-like
    return pathRet / ".superman";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", SUPERMAN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Superman Core copyright is not removed by accident
    if (copyright_devs.find("Superman Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Superman Core developers";
    }
